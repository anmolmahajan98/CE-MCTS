...
for index in range(0, 5):
    ...
    for i in range(0, 640):
        ...
        original_labels.append(Y_test[i][0])
    ...
    for i in range(0, 640, 10):
        ...
        current_original_labels = []
        for j in range(i, i+10):
            current_original_labels.append(original_labels[j])
            current_predicted_labels.append(predicted_labels[j])
        mse = mean_squared_error(current_original_labels, current_predicted_labels)
        mse_vals.append(abs(mse))
        mse_total += abs(mse)

    print(mse_total)
    print(float(mse_total/64))
    mse_vals_numpy = np.array(mse_vals)
    print(np.std(mse_vals_numpy))

    all_mse_total+=mse_total
    all_mse_avg+=float(mse_total/64)
